...
async def test_add_related_returns_201(
    authenticated_client: AsyncClient, concepts: list[Concept]
) -> None:
...
async def test_add_related_duplicate_returns_409(
    authenticated_client: AsyncClient, db_session: AsyncSession, concepts: list[Concept]
) -> None:
...
async def test_add_related_different_scheme_returns_400(
    authenticated_client: AsyncClient, concepts: list[Concept], concept_other_scheme: Concept
) -> None:
...
async def test_remove_related_returns_204(
    ...
    response = await authenticated_client.delete(f"/api/concepts/{dogs.id}/related/{cats.id}")
...
async def test_get_concept_includes_related_in_response(
    ...
    response = await authenticated_client.get(f"/api/concepts/{dogs.id}")
...
async def test_get_concept_related_is_symmetric(
    ...
    response = await authenticated_client.get(f"/api/concepts/{cats.id}")
