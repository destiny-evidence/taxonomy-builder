...
If the builder cannot represent the Condition hierarchy ([#109](https://github.com/destiny-evidence/taxonomy-builder/issues/109)), it cannot author the classes that `evaluates` and `comparedTo` point to. If it cannot represent union domains ([#110](https://github.com/destiny-evidence/taxonomy-builder/issues/110)), it cannot express that `supportingText` applies to both CodingAnnotation and ObservedResult. The 3ie example makes these gaps concrete: the CodingAnnotation provenance fields (`supportingText`, `codedBy`, `status`) show the full chain from flat tabular data through vocabulary concepts to the nested evidence graph.
